...
long 
api_extract_long(int fmt,void * v, int i)
{
    switch (fmt) {
    case BYTE:
        return ((long) ((u_char *) v)[i]);
    case SHORT:
        return ((long) ((short *) v)[i]);
    case INT:
        return ((long) ((int *) v)[i]);
    case LONG:
        return ((long) ((long *) v)[i]);
    case FLOAT:
        return ((long) ((float *) v)[i]);
    case DOUBLE:
        return ((long) ((double *) v)[i]);
    }
    return (0);
}
float 
...
typeconvert_args(int srctype,void *src,int dsttype,void *dst,int dsize){
	...
	lp = (long *) dst;
	for(lup = 0; lup < dsize; lup++){
	    lp[lup] = api_extract_long(srctype,src, lup);
	}
	break;
    case FLOAT:
	fp = (float *) dst;
