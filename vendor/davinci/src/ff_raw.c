...
ff_raw(vfuncptr func, Var * arg)
{
	...
	const char *formats[] = { 
		"byte", "msb_short", "msb_int", "msb_long", "msb_float", "msb_double" ,
		        "lsb_short", "lsb_int", "lsb_long", "lsb_float", "lsb_double" , NULL
		};
	...
	if (!strcmp(format, "byte")) {
		...
		h.format = iom_INT;
	} else if (!strcmp(format, "msb_long")) {
		h.eformat = iom_MSB_INT_8;
		h.format = iom_LONG;
	} else if (!strcmp(format, "lsb_long")) {
		h.eformat = iom_LSB_INT_8;
		h.format = iom_LONG;
