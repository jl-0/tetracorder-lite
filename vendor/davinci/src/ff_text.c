...
Var *ff_delim_textarray(Var * ob, int item, char *delim)
{
  int i;
  int j;
  int Row;
...
Var *textarray_subset(Var * v, Var * range)
{
  ...
  for (i = lo[1]; i <= hi[1]; i += step[1]) {
    if (lo[0] >= strlen(V_TEXT(v).text[i])) {
      ...
      if ((hi[0] - lo[0] + 1) > strlen(V_TEXT(v).text[i])) {
        V_TEXT(o).text[counter] =
            strdup((V_TEXT(v).text[i] + lo[0]));
      } else {
        V_TEXT(o).text[counter] =
            (char *) calloc((hi[0] - lo[0] + 1) + 1, sizeof(char));
        memcpy(V_TEXT(o).text[counter],
               (V_TEXT(v).text[i] + lo[0]), (hi[0] - lo[0] + 1));
...
char *string_basename(Var * ob1, char *ext)
{
  char *s, *q;
...
Var *text_basename(Var * ob1, char *ext)
{
  int i;
  Var *S = newVar();
  Var *Tmp = newVar();
...
Var *ff_text_strstr(Var * ob1, char *s1)
{
  int i;
...
Var *set_text(Var * to, Range * r, Var * from)
{
  Var *src;
  Var *dest;
  ...
  if (V_TYPE(from) == ID_STRING) {
    ...
    if (((hi[1] - lo[1] / step[1] + 1) != V_TEXT(from).Row)
        && (V_TEXT(from).Row > 1)) {
  ...
  if (hi[1] >= V_TEXT(to).Row) {
    parse_error("Row out of range");
    return(NULL);
  }
...
Var *where_text(Var * id, Var * where, Var * exp)
{
  int i;
  Var *temp;
  int len = 0;
  ...
  if (V_TEXT(id).Row != V_SIZE(where)[1]) {
    parse_error
        ("Target and source need to have the same number of rows");
    return (NULL);
  }
        ...
        if (i < (len - 1)) {
          ...
          if (replace[i] >= '0' && replace[i] <= '9') {       /*substring */
            ...
            if (pmatch[numeral].rm_so >= 0) {       /*requested valid substring */
              delta =
                  pmatch[numeral].rm_eo -
                  pmatch[numeral].rm_so;
              memcpy((newtext + index),
                     Marker + pmatch[numeral].rm_so, delta);
              index += delta;
            }
...
Var *ff_create_text(vfuncptr func, Var * arg)
{
  ...
  if (count < 0) {
    parse_error("Invalid argument to %s.  %s must be greater than 0.",
