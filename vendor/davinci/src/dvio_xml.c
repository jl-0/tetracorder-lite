...
static char *get_name(xmlNodePtr nptr)
{
    ...
    while (cur != NULL)
    {
        if (!strcmp((char *) cur->name, NAME_ATTRIBUTE))
        {
            ...
            if (cur->children->content != NULL && cur->children->type
                    == XML_TEXT_NODE)
            {
                ...
                if (name == NULL)
                {
                    ...
                    parse_error(
                            "Memory allocation error processing an XML node %s\n",
                            (char *) nptr->name);
                    return NULL;
                }
                break;
...
static int load_node(xmlNodePtr nptr, Var *v, int use_names)
{
    ...
    if (nptr->type == XML_ELEMENT_NODE && nptr->name != NULL)
    {
        ...
        if (!strcmp(node_name, FIELD_BINARY))
        {
            ...
            {
                node_name = gen_next_unused_name_instance(node_name, v, 1);
            }
            is_field_bin = 1;
        }
        ...
        else if (!strcmp(node_name, TABLE_BINARY))
        {
            if (use_names)
            {
                ...
                if (node_name == NULL)
                {
					node_name = gen_next_unused_name_instance(TABLE_BINARY, v, 1);
                }
                is_table_bin = 1;
            }
            else
        ...
        else if (use_names && !strcmp(node_name, NAME_ATTRIBUTE) && (!strcmp(
                (char *) nptr->parent->name, TABLE_BINARY) || !strcmp(
                (char *) nptr->parent->name, FIELD_BINARY)))
        {
            goto NEXT_NODE;
        }
        ...
        if (child_cnt < 1)
        {
            ...
            if (node_content != NULL)
            {
                ...
                if (elem != NULL)
                {
                    ...
                    {
                        parse_error(
                                "Error adding a 'value' sub-struct to %s\n",
                                node_name);
                        if (node_name != NULL)
                        {
                            free(node_name);
        ...
        else
        {
            if (child_cnt > 0)
            {
                ...
                {
                    add_struct(elem, "element_type", newString(strdup(
                            TABLE_BINARY)));
                }
 ...
 */
static Var * load_xml_struct(char *filename, xmlDocPtr doc, Var *v,
        int use_names)
{
    xmlAttrPtr attr = NULL;
 ...
 */
static Var *
load_xml_func(const char *filename,
        xmlDocPtr doc /* the resulting document tree */, Var *v, int use_names)
{
    ...
    doc
            = xmlCtxtReadFile(ctxt, filename, NULL, XML_PARSE_DTDATTR /* XML_PARSE_DTDVALID */);
 ...
 */
Var *
dv_LoadXML(char *filename, int use_names)
{
    Var *v = NULL;
    xmlDocPtr doc;
