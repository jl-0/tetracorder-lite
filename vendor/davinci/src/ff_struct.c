...
ff_remove_struct(vfuncptr func, Var * arg)
{
	...
	if (name != NULL) {
		if (find_struct(a, name, NULL) < 0) {
			...
			// TODO could it be fixed by doing a mem_claim(s) instead of V_DUP
...
Var *
remove_struct_by_key(Var *s, const char *key)
{
	if (find_struct(s, key, NULL) < 0){
		return NULL;
	}

	return Narray_delete(V_STRUCT(s), key);
}
