...
pp_math(Var * a, int op, Var * b)
{
    ...
    **/
	...
	if (math_operable(a,b) == 0) {
        parse_error("math operation illegal, sizes differ on more than 1 axis or an axis is zero");
        return (NULL);
	}
    /**
...
/** size was extracted as x,y,z.  put it back appropriately **/
    ...
    if (is_relop(op)) {
        ...
        case INT:
            ...
            break;
        case LONG:
            DO_RELOP_LOOP(long, u_char, extract_long, (long));
            break;
        case FLOAT:
		...
		case LONG:
			...
			case INT:
				...
				break;
			case LONG:
				DO_SHIFT_LOOP(long, long, extract_long, (long));
				break;
			}
			break;
		default:
        ...
        **/
        ...
        case INT:
            ...
            break;
        case LONG:
            DO_MATH_LOOP(long, long, extract_long, (long));
            break;
        case FLOAT:
...
*/
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (va == 0 || vb == 0) {
            parse_error("math operation illegal, one or more axes is zero");
            return (0);
		}
        if (va != vb) {
            ca *= va;
...
long int 
extract_long(const Var * v, const size_t i)
{
	long int ii;
	double dd;
    ...
    switch (V_FORMAT(v)) {
    case BYTE:
        return ((long int) ((u_char *) V_DATA(v))[i]);
    case SHORT:
        return ((long int) ((short *) V_DATA(v))[i]);
    case INT:
        return ((long int) ((int *) V_DATA(v))[i]);
    case LONG:
        return ((long int) ((long *) V_DATA(v))[i]);
    case FLOAT:
        return ((long int) ((float *) V_DATA(v))[i]);
    case DOUBLE:
        return ((long int) ((double *) V_DATA(v))[i]);
    }
    return (0);
}
float 
