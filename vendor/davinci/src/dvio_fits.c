...
Read_FITS_Image(fitsfile *fptr) {
	...
	case SBYTE_IMG: // TODO is BYTE and TSBYTE the right things to use?
		format = BYTE;
		datatype = TSBYTE;
		break;
	...
	case USHORT_IMG: // TODO is SHORT and TSHORT the right things to use?
		format = SHORT;
		datatype = TSHORT;
		break;
...
Read_FITS_Table(fitsfile *fptr) {
	...
	int colnum = 0, fmt;
	char *ptr, **pptr, **qqptr;
	Var *data = NULL, *element, *e4d = NULL;
	...
	while (col_status != COL_NOT_FOUND && colcount < ncols) {
		colcount++;
		...
		// TODO handle COMPLEX case for 3 and 4d
		case TCOMPLEX:
			datatype = TFLOAT;
			fmt = FLOAT;
			x = repeat;
		...
		// decode TDIM keyword, see: https://fits.gsfc.nasa.gov/users_guide/users_guide/node69.html
		naxes[0]=naxes[1]=naxes[2]=naxes[3]=1;
		fits_read_tdim(fptr, colnum, 4, &naxis, naxes, &status);
		sprintf(msg, " getting column %d tdim", colnum);
		QUERY_FITS_ERROR(status, msg, NULL);

		if (naxis > 3){
			parse_error("WARNING! tdim for column %d has more %d dimensions which is more than we can handle (4)\n", colnum, naxis);
		}
		...
		if (ptr != NULL) {
			...
			if (datatype == TSTRING) {
				...
				if (naxis <= 2){
					...
					// TODO do we need fits_read_subset()?
					w = y; x = naxes[0]; y = naxes[1]; z = naxes[2];
					element = new_struct(w);
					for(r=0; r<w; r++){
						fits_read_col(fptr, datatype, colnum, r+1, 1, x * y * z, NULL,
								ptr, NULL, &status);
						sprintf(msg, " reading column %d data (4th dim = %d)", colnum, w);
						QUERY_FITS_ERROR(status, msg, NULL);
						e4d = newVal(BSQ, x, y, z, fmt, ptr);
						insert_struct(element, r, NULL, e4d);
						if (r<(w-1)){
							// allocate space for (w+1)th element
							ptr = calloc(x * y * z, NBYTES(fmt));
						}
					}
				}
