            ...
            for (k = 0 ; k < z ; k++) {
                for (j = 0 ; j < y ; j++) {
                    for (i = 0 ; i < x ; i++) {
                        ...
                        case LONG:  printf("%ld\t", ((long *)V_DATA(v))[c]); break;
            ...
            for (i = 0 ; i < size[0] ; i++) {
				...
				*/
                ...
                if (V_FORMAT(dst) == V_FORMAT(src)){
                    ...
                    case LONG:
                        ((long *)V_DATA(dst))[d] =
                            saturate_long(extract_long(src, s));
                        break;
                    case FLOAT:
                        ((float *)V_DATA(dst))[d] =
                            extract_float(src, s);
...
pp_set_struct(Var *a, Var *b, Var *exp)
{
    ...
    if (V_TYPE(a) != ID_STRUCT) {
        parse_error("Argument %s is not a struct\n", V_NAME(a) != NULL? V_NAME(a): "");
        return(NULL);
    }
    ...
    if (V_DSIZE(exp) == 1) {
        ...
        for (i = 0 ; i < dsize ; i++) {
            ...
            if (extract_int(where, j)) {
                ...
                case LONG:		((long *)V_DATA(id))[i] = lval; break;
        ...
        for (i = 0 ; i < dsize ; i++) {
            ...
            if (extract_int(where, j)) {
                ...
                case INT:		
                    ...
                    break;
                case LONG:		
                    ((long *)V_DATA(id))[i] = extract_long(exp, k); 
                    break;
                case FLOAT:		
