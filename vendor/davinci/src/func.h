...
long int extract_long (const Var * v, const size_t i);
...
extern "C" void parse_error2(const char *fmt, ...);
#else
...
char *trim(char *str, const char *opt_chars); // pass opt_chars = NULL to use isspace()
char *fix_name(const char *input_name, int locase); // locase=1 lowercases the name

char *gen_next_unused_name_instance(char *keyname, Var *s, int first_instance_special); // generate unused key name from keyname in structure "s"
...
extern "C" {
  ...
  Var * remove_struct_by_key(Var *s, const char *key);
