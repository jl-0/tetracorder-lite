...
/**
 ** ff_dfunci() - function caller for intrinsic math (double) functions
 ** that return boolean values.
 **
 ** This function iterates on all the elements of the passed argument,
 ** calling a named function for each element.  The named functions can
 ** only handle single value arguments (ie: is_nan(), is_inf(), etc)
 **
 ** return value should be integer.
 **/

Var *
ff_dfunci(vfuncptr func, Var * arg)
{
  Var *v = NULL, *s;
  dfunci fptr;
  int *idata;
  ...
  idata = (int *) data;
  for (i = 0; i < dsize; i++) {
    idata[i] = (int) fptr(extract_double(v, i));
  }
...
ff_conv(vfuncptr func, Var * arg)
{
  ...
  switch (format) {
    ...
    case LONG:
      {
        long d;
        long *idata = (long *) data;
        for (i = 0; i < dsize; i++) {
          d = extract_long(v, i);
          idata[i] = saturate_long(d);
        }
        break;
      }
    case FLOAT:
...
ff_format(vfuncptr func, Var * arg)
{
  ...
  const char *formats[] = { "byte", "short", "int", "long", "float", "double", NULL };
  ...
  if (format_str == NULL) {
    ...
    else if (!strcasecmp(format_str, "long")) ptr = (char *)"long";
...
ff_create(vfuncptr func, Var * arg)
{
  ...
  if (format_str != NULL) {
    ...
    else if (!strcasecmp(format_str, "long")) format = LONG;
    ...
    if (step == 0){
      if (dsize > 0){
        ...
        switch (format) {
          ...
          case LONG: ldata[0] = saturate_long(v); break;
      ...
      for (i = 0 ; i < V_DSIZE(a) ; i++) {
        switch(format) {
          ...
          case INT:
            ...
            break;
          case LONG:
            if (extract_long(a,i) != extract_long(b,rpos(i,a,b)))
              return(0);
            break;
          case FLOAT:
...
ff_contains(vfuncptr func, Var * arg)
{
  ...
  switch (V_FORMAT(obj)) {
    ...
    case INT:
      ...
      break;
    case LONG:
      vl = extract_long(value, 0);
      for (i = 0 ; i < dsize ; i++) {
        if (((long *)V_DATA(obj))[i] == vi) {
          ret = 1;
          break;
        }
      }
...
int my_isnan(double d){ return isnan(d); }
int my_isinf(double d){ return isinf(d); }
