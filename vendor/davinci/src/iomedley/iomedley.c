...
{
    if (h->ddfname) { free(h->ddfname); h->ddfname = NULL; }
...
FILE *
iom_uncompress(FILE * fp, const char *fname)
{
        /**
...
char *
iom_uncompress_with_name(const char *fname)
{
        /**
         ** Try gzip first, then compress
         **/
...
char *
iom_expand_filename(const char *s)
{
    char buf[4096];
    char ebuf[2048];
    char *p, *q, *e;
