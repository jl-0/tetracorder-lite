...
#define SIGNED_MSB_LONG 	'L'
#define SIGNED_LSB_INT		'i'
#define UNSIGNED_LSB_INT	'u'
#define SIGNED_LSB_LONG	    'l'
...
static int convert_types(data* thedata, int num_items, int rows)
{
	...
	for(i=0;i<num_items;i++) {
		switch(thedata[i].type)
		{
			case SIGNED_LSB_INT:
			case SIGNED_MSB_INT:
			case SIGNED_LSB_LONG:
			case SIGNED_MSB_LONG:
				switch(thedata[i].numbytes){
				case 2: thedata[i].type = SHORT; break;
				default:
				case 4: thedata[i].type = INT; break;
				case 8: thedata[i].type = LONG; break;
				}
				break;
		...
		for(j=0;j<num_items;j++)
		{
			...
			for(k=0;k<columns;k++)
			{
				switch(the_data[j].type)
				{
					...
					case SIGNED_LSB_INT:
						if(bytesize == 8) {
							memcpy(&tempint, &the_data[j].array[(i*columns+k)*8], 8);
							printf("%ld\t",tempint);
						}
						else if(bytesize == 4) {
					...
					case UNSIGNED_LSB_INT:
						if(bytesize == 8) {
							memcpy(&utempint, &the_data[j].array[(i*columns+k)*8], 8);
							printf("%lu\t",utempint);
						}
						else if(bytesize == 4) {
				...
				case SIGNED_LSB_INT:
					...
					if(letter == SIGNED_LSB_INT) {
						for(i=0;i<numbytes;i++)
							mybyte[(sizeof(tempint)-1)-(sizeof(tempint)-numbytes)-i] = buf[i];
						tempint = tempint >> ((sizeof(tempint)-numbytes)*8);
					} else {
						for(i=0;i<numbytes;i++)
							mybyte[i] = buf[i];	
						tempint = tempint >> ((sizeof(tempint)-numbytes)*8);
					...
					memcpy(loc, &mybyte[sizeof(tempint)-al_bytes], al_bytes);

				#else
					if(letter == SIGNED_MSB_INT) {
						for(i=0;i<numbytes;i++)
							mybyte[(sizeof(tempint)-1)-i] = buf[i];
						tempint = tempint >> ((sizeof(tempint)-numbytes)*8);
					} else {
						for(i=0;i<numbytes;i++)
							mybyte[(sizeof(tempint)-1)-(numbytes-1)+i] = buf[i];	
						tempint = tempint >> ((sizeof(tempint)-numbytes)*8);
					}
...
/*
	...
	and signed shorts and ints.  So this checks and possibly converts the following:
		...
		unsigned ints	->		longs  (required conversion)
*/
			...
			case UNSIGNED_LSB_INT:
				if( bytesize==4 ) {
					...
					if( j < rows*columns ) {
						...
						the_data[i].type = SIGNED_LSB_LONG;
...
// TODO implement packing a single object without having to first put it in a struct
...
convert_to_ext_fmt(char *from, int ffmt, char *to, int tfmt, int tolen){
    ...
    switch(ffmt){
    case BYTE:      ul = *(unsigned char *)from; sl = (int         )ul;  d  = (double)ul;  break;
    case SHORT:     sl = *(short         *)from; ul = (unsigned int)sl;  d  = (double)sl;  break;
    case INT:       sl = *(int           *)from; ul = (unsigned int)sl;  d  = (double)sl;  break;
    case LONG:      sl = *(long          *)from; ul = (unsigned long)sl; d  = (double)sl;  break;
    case FLOAT:     d  = *(float         *)from; ul = (unsigned long)d;  sl = (long   )d;  break;
    case DOUBLE:    d  = *(double        *)from; ul = (unsigned long)d;  sl = (long   )d;  break;
    case ID_STRING: d  = atof(from); ul = strtoul(from,NULL,10); sl = strtol(from,NULL,10);  break;
    default: return 0;
    }
    ...
    if (tfmt == STRING){
        switch(ffmt){
        case BYTE:      sprintf(str,"%lu",ul); break;
        case SHORT:
        case INT:
        case LONG:      sprintf(str,"%ld",sl); break;
        case FLOAT:
        case DOUBLE:    sprintf(str,"%g",d); break;
        case ID_STRING: snprintf(str,tolen,"%.*s",tolen,from); break;
        default:        return 0;
        }
    }
    ...
    case SIGNED_LSB_INT:
        ...
        case 8:      copy(&sl, to, sizeof(sl), big^0); break;
        default:     return 0;
        }
        break;
    case SIGNED_MSB_INT:
        ...
        case 8:      copy(&sl, to, sizeof(sl), big^1); break;
        default:     return 0;
        }
        break;
    case UNSIGNED_LSB_INT:
        ...
        case 8:      copy(&ul, to, sizeof(ul), big^0); break;
        default:     return 0;
        }
        break;
    case UNSIGNED_MSB_INT:
        ...
        case 8:      copy(&ul, to, sizeof(ul), big^1); break;
        default:     return 0;
        }
        break;
