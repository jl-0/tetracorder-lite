...
struct _vfuncptr vfunclist[] = {
  ...
  /*
   * By using the third argument to hold a pointer to an actual library
   * function, we can reuse ff_dfunci() to implement ANY function that
   * expects a single double argument, and returns an integer 
   * argument (like some simple math functions).
   */
  { "isnan",      ff_dfunci,    (void *)my_isnan,   NULL},
  { "isinf",      ff_dfunci,    (void *)my_isinf,   NULL},
