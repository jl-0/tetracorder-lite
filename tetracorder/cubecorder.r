...
# arrays for buffering output
	...
	integer*4 ihour, imm, deg, minute, mm, dd, yy, ierg
	...
	integer*4 debugyel   # debug line number
	integer*4 debugx1    # debug sample from debugx1 to debugx2
	integer*4 debugx2

# set up debugging lines.  set debugyel = -1 to turn of debugging geologic origin 

	# yel == 604 & xel > 195 is for cuprite 1995 aviris cube over the hydrothermal area.
	debugyel = 604   # line
	debugx1  = 195   # sample 1
	debugx2  = 225   # sample 2
...
# Get 3d file id
	...
	if (i >= 80 & f3did == 0) go to 4      # blank line, read again
...
243			format (/,'Enter the DN offset, ',
			...
			i = 1
			call wjfren (i,x,il)
			if (i >= 80 & il == 0) go to 242    # blank line, read again
...
################
	...
	ier = 0
	# call opentf (ier)     # this is for the future
	if (ier != 0) {
...
################
#
# now open the geologic origins cube and create header
#
################

	# geoflag:
	#  0 = no geologic origins
        #  1 = geologic origins enabled by user input
        #  2 = geologic origins files read
        #  3 = 2 + geologic origins cube opened

	if (geoflag > 1) {   # should be 2 at ths point if doing geologic origins
		ier = 0
		call opengeocub (ier)     # origins cube
		if (ier != 0) {
			write (ttyout, *) 'ERROR OPENING or writing to the geologic origins cube.   PRESS RETURN TO EXIT'
			call what (-1)
			call crtin
			return
		}
		geoflag = 3   # all good
		write (ttyout, *) 'geologic origins setup complete: geoflag=', geoflag,'   including geologic origins calculations'
	}
...
# Loop to extract spectra from 3d data file and compute band depth
	...
	if (cmdverbose(-1) <= 2) write (ttyout,*) 'geology: geoflag=', geoflag,'  (geoflag must be > 2 to do geologic origins calculations)'
...
#
		...
		# zero geologic origin array for the sheet
		do iixel = 1,dx { 
			do iig = 1, geochans {

				geocub(iixel,iig) = 0.0
			}
		}
...
#		write(*,*) 'DEBUG: ', icubflg, specdat(nchans/2,xel)
			...
			do iig = 1, geochans {   # zero the count
				ngeocnt(xel,iig) = 0.0
			}
			...
			do jtmp = 1, nzgroup {    # store best match for each
			    ...
			    if ( groupenable(jtmp) > 0 ) {      # only do the following if the group is enabled

				igbest(jtmp,xel) = grpbest(jtmp)  # best answer in this group (imat pointer)

#		write(*,*) 'DEBUG: best match', yel, xel, igbest(jtmp,xel) # material No.

				# now do geologic origins
				if (geoflag > 2) {

					igeobest = grpbest(jtmp)   # best answer in this group (imat pointer)
					#write (ttyout,*) ' '
					#write (ttyout,*) 'Debug: 0  line=',yel,' sample=',xel,' group=',jtmp,' igeobest=',igeobest
					if (igeobest < 1 | igeobest > nmats) {

						igeobest = 1
						#write (ttyout,*) 'Debug: 0a: resetting igeobest to 1, was: ',igeobest
					}

					inmatcomps = nmatcomponent(igeobest)  # numner of material components per reference spectrum
					do iigm = 1, inmatcomps {

						imatindex1 = matcompgeorg(iigm, igeobest)   # component index
						#write (ttyout,*) 'Debug: 1  line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm,' imatindex1=',imatindex1

						if (imatindex1 > 0 & imatindex1 <= ngmaterials ) {

							igeiindex1 = matcompgeorg(iigm,igeobest)  # material component index to the geologic origin
							#write (ttyout,*) 'Debug: 1b material component index igeiindex1=',igeiindex1
							if (igeiindex1 < 1) {

								#write (ttyout,*) 'Debug: 1c: igeiindex1 <1, reset ti1'
								igeiindex1 = 1
							}
						} else {
							#write (ttyout,*) 'Debug: line=',yel,' sample=',xel,' group=',jtmp,' igeiindex1 set to 1, was:',igeiindex1
							imatindex1 = 1
							igeiindex1 = 1
						}
						...
						do iig = 1, geochans {   # normalize the origins
							jjx(iig) =  int(geoorigin(iig, igeiindex1) + 0.5)

							if (yel == debugyel & xel > debugx1 & xel < debugx2 & groupenable(jtmp) > 0 ) {

								write (ttyout,*) 'Debug: 3a line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm,
								' before dividing by ngeocnt: geocub(',xel,',',iig,')= ', geocub(xel,iig),
								'  ngeocnt(',xel,',',iig,')= ',ngeocnt(xel,iig), ' group=',jtmp
							}
							geocub1(xel,iig) = geocub(xel,iig)  # geocub before applying ngeocnt
							if (yel == debugyel & xel > debugx1 & xel < debugx2 & groupenable(jtmp) > 0 ) {
								write (ttyout,*) 'Debug: 3a line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm,' igeobest=',igeobest,' igeiindex1=',igeiindex1
								write (ttyout,*) 'Debug: 3a: geoorigin(iig,igeiindex1)=(',iig,',',igeiindex1,')',geoorigin(iig, igeiindex1)
								write (ttyout,*) 'Debug: 3a:      ofit=',ofit(igeobest,xel),'   depth=',odepth(igeobest,xel),'  ',matcomponent(iigm,igeobest)(1:30)
								write (ttyout,*) 'Debug: 3a: matidconfidence=',matidconfidence(igeobest)
								write (ttyout,*) 'Debug: 3a: geoorigin(',iig,',',igeiindex1,')= ',geoorigin(iig, igeiindex1)
								write (ttyout,*) 'Debug: 3a: ngeocnt(',xel,',',iig,')= ',ngeocnt(xel,iig),' geocub(xel,iig)=',geocub(xel,iig),' line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm
								write (ttyout,*) 'Debug: 3a: line=',yel,'  geocub(',xel,',',iig,')= ', geocub(xel,iig),' group=',jtmp,'  ngeocnt(',xel,',',iig,')= ',ngeocnt(xel,iig)
								write (ttyout,*) ' '
							}
						}

						#write (ttyout,*) 'Debug: 2 line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm
						##########################  debug
						# debugyel, debugx1, debugx2 are set at the top of the subroutine
						if (yel == debugyel & xel > debugx1 & xel < debugx2 & groupenable(jtmp) > 0 ) {

							if (jtmp == 1) write (ttyout,*) ' '
							write (ttyout,*) ' '
							write (ttyout,*) 'Debug: 3 line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm

							write (ttyout,7783) yel, xel, jtmp, igeobest, 
								iigm, matcomponent(iigm,igeobest)(1:30)

7783			format('DEBUG material components: line=',i5,' sample=',i5,' group=',i3,' best matrl=',i3,
								 ' matrl component=', i2,' ',a30)

						#### the problem here is igeiindex1 is incorrect 3/16/2026, 8:42am
							#write (ttyout,*) 'Debug: 4'
							write (ttyout,7784) yel, xel, jtmp,
								igeobest, iigm,
								gmaterials(igeiindex1)(1:30), igeiindex1, jjx
									

7784			format ('DEBUG geocub: line=',i5,' sample=',i5,' group=',i3,' best matrl=',i3,
				' matrl component=',i2,' ',a30,'  geoindex=',i4,' geo origin= ',
				i2,' ',i2,' ',i2,' ',i2,' ',i2,' ',i2,' ',i2,' ',i2,' ',i2,' ',i2,' ',i2)
							do iig = 1, geochans { 
								write (ttyout,*) '    Debug: 3: before normalizing with ngeocnt:   geocub(',xel,',',iig,')= ',
									geocub(xel,iig),'  ngeocnt(',iig,')= ',ngeocnt(xel,iig),
									' geocub1(xel,iig)=',geocub1(xel,iig)
							}
							write (ttyout,*) ' '
							write (ttyout,*) '  #######  '
							write (ttyout,*) ' '
						}   # end of debuf write block

						#write (ttyout,*) 'Debug: 4 line=',yel,' sample=',xel,' group=',jtmp,' iigm=',iigm

					}  # end of materials in each reference spectrum loop
				    }     # end of gepf;ag > 2 if statement
				}  # end of group enable test

			}          # end of grup loop (xel up to  dx)

			do iig = 1, geochans {
				if (ngeocnt(xel,iig) > 0.0001) {
					geocub(xel,iig) = geocub(xel,iig) / ngeocnt(xel,iig)
				} else {
					geocub(xel,iig) = 0.0
				}
				if (yel == debugyel & xel > debugx1 & xel < debugx2 ) {
					write (ttyout,*) 'Debug: 3b: line=',yel,'  geocub(',xel,',',iig,')= ', geocub(xel,iig),'  ngeocnt(',xel,',',iig,')= ',ngeocnt(xel,iig)
				}
			}

		}  # end analysis of one spectrum, (xel up to  dx)
...
#			write(*,*) 'DEBUG: ', igroup, imat, matgrp(imat, igroup)
				...
				if (ier != 0) {    # note ier is passed in common
					...
					write (ttyout,*) 'cublineout write ERROR= ', ier
					write (ttyout,*) 'Press return to exit'
					call crtin
					return
				}
		...
		############### now output geologic origins ########################

		if (geoflag > 2) {   # geologic origins enabled

			do iig = 1, geochans {   # write line at channel iig

				do xel = 1,dx {     # Process each geologic origins line at a time in the sheet

					geocubi2(xel) = int(geocub(xel,iig) * 1000.0)
					if (geocubi2(xel) < 0) geocubi2(xel) = 0

					if (yel == debugyel & xel > debugx1 & xel < debugx2 ) {

						write (ttyout,*) 'geocube output: line ',yel,' sample ',xel,' channel iig= ',iig,' geocub(xel,iig)=',geocub(xel,iig),' geocubi2(xel)= ',geocubi2(xel)
					}
				}
				igkey = (yel-1)*geochans + iig + 1   # actually should be number of vicar header records instead of 1

				# write (ttyout,881) yel, iig, dx, igkey

881 				format ('DEBUG: writing geocube, image row=',i5,'   geochan=',i3,
						'   16-bit pixels=',i5,'    record=',i5)

				write (lungeocube, rec=igkey, iostat=ier) geocubi2(1:dx)
				if (ier != 0) {   # Check for error in writing

					write (ttyout,*) ' ERROR writing geocube, line= ',yel,' origins chan= ',
							iig,'  error= ', ier
					call what (-1)
					stop
				}
	...
	}   # end of yel loop (image row), thus one sheet is complete
