...
##########################################################
# list spectrally active components
##########################################################

541     if (cmdverbose(-1) <= 1) write (ttyout,542)
542	format (///,' type: list materials',/)
              #                      1111111111222
	...
	i = 1
	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 541    # blank line, read again
	if (il==ihx || il==ihe)  {
		ic=il
		icrst = 1
		call rstart(icrst)
	...
	# 
	i = i -1
	if (iopcon(i:i+13) != 'list materials') {
...
545	write (ttyout,546) maxmatcomps
546	format(' Type one per line:',/,
	' material_name   class_name   abundance (optional)',/,
	'                 (maximum ',i4,' components',/,
	' or end_list_materials to end this section',/)

	call crtin
	if (cmdverbose(-1) <= 2) write (ttyout,*) iopcon
	i = 1
	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 545    # blank line, read again
	if (il==ihx )  {
		ic=il
		icrst = 1
		call rstart(icrst)
	...
	if (iopcon(i-1:i) == 'e ')  {   # checks for e by itself
		ic=il
		icrst = 1
		call rstart(icrst)
		call what (-1)
	...
	# 
	i = i - 1   # note: after the wjfren call above, i will be greater than 1, pointing to the second character on the line
	for (kcomps=1; kcomps <= maxmatcomps; kcomps = kcomps+1) {


		write (ttyout,*) ' component material ',kcomps,' :'
		if (kcomps > 1) {
...
548			call crtin
			...
			i = 1
			call wjfren (i,x,il)
			if (i >= 80 & il == 0) go to 548    # blank line, read again
			i = i - 1
		}

		if (iopcon(i:i+17) == 'end_list_materials') {    # endf of listing materials
                                   #            111111111
                                   #   123456789012345678
		...
		kbl = index(iopcon(i:i+30), ' ')   # find end of word
		if (kbl == 0) kbl = i+30           # all characters are non blank
		kbl = kbl -1
		matcomponent(kcomps, imat)(1:kbl) = iopcon(i:i+kbl)    # materials component
		...
		if (kbl == 0) kbl = i+30           # all characters are non blank
		kbl = kbl -1
		matcompclass(kcomps, imat)(1:kbl) = iopcon(i:i+kbl)    # materials component class

		##### material component abundance

		i = i+kbl +1
		call wjfren (i,x,il)
		if ( x > 0.0 ) {
			matcomponentabundance(kcomps, imat) = x
		}
		nmatcomponent(imat) = kcomps   # number of components

		#### write a history line    ZZZZZZ to be done
		write (ttyout,*) ' material: ',imat,' component: ',kcomps,' ',matcomponent(kcomps, imat),
			' class: ',matcompclass(kcomps, imat),' abund: ',matcomponentabundance(kcomps, imat) 

	}

	ikcomps = nmatcomponent(imat)

	do mmg = 1, ikcomps {    # iterate over number of materials in the reference spectrum

		# now determine index to material classifications table   geologic-origins/material-classification.txt

		matcomponidx(mmg,imat) = 1  # default = xero (first line in the table should be zero)

		do mcm = 1, nmatnames {

			if (matcomponent(mmg,imat)(1:30) == matname(mcm)(1:30) ) {

				matcomponidx(mmg,imat) = mcm   # index pointing to the material name

				write (ttyout,7771) imat, mmg, mcm, matcomponent(mmg,imat)(1:30), matname(mcm)(1:30)
7771				format ('DEBUG: classification: imat=',i4,' mmg=',i3,' mcm=',i3,
					' MATCH: matcomponent= ',a,' == matname= ',a)
			} else {

				idebug = 0 # do something
				#write (ttyout,7772) imat, mmg, mcm, matcomponent(mmg,imat)(1:30), matname(mcm)(1:30)
7772				format ('DEBUG: classification: imat=',i4,' mmg=',i3,' mcm=',i3,
					' NO match: matcomponent= ',a,' != matname= ',a)
			}
		}

		imindx = matcomponidx(mmg,imat)
		write (ttyout,321) imat, mmg, matname(imindx)(1:30),
			matclassname(imindx)(1:20), matgroupname(imindx)(1:20)
321		format('material ',i5,' component:',i5,3x,a,2x,a,2x,a)

		# now determine index to geologic origins table  geologic-origins/geological-origin-table-by-material.txt

		matcompgeorg(mmg,imat) = 1   # default = xero (first line in the table should be zero)

		do mcm = 1, ngmaterials {

			if (matcomponent(mmg,imat)(1:30) == gmaterials(mcm)(1:30) ) {

				matcompgeorg(mmg,imat) = mcm   # index pointing to the geologic origins table
				write (ttyout,7781) imat, mmg, mcm, matcomponent(mmg,imat)(1:30), gmaterials(mcm)(1:30)
7781				format ('DEBUG: geo origins: imat=',i4,' mmg=',i3,' mcm=',i3,
					' MATCH: matcomponent= ',a,' == gmaterials= ',a)
			} else {

				idebug = 0 # do something
				# write (ttyout,7782) imat, mmg, mcm, matcomponent(mmg,imat)(1:30), gmaterials(mcm)(1:30)
7782				format ('DEBUG: geo origins: imat=',i4,' mmg=',i3,' mcm=',i3,
					' NO match: matcomponent= ',a,' != gmaterials= ',a)
			}

		}
		do mm = 1, geochans {
			jjx(mm) =  int(geoorigin(mm,imat) + 0.5)
		}
		# was: igindx = gmaterials(matcompgeorg(mmg,imat))
		igindx = matcompgeorg(mmg,imat)

		write (ttyout,322)  imat, mmg, igindx,
			matcomponent(igindx,imat)(1:30), jjx

322		format ('Geologic origin:',/,
                  'imat=',i5,' component=',i2,' igindx=',i5,' ',
		  a30,' ',i2,' ',i2,' ',
...
564	if (cmdverbose(-1) <= 1) write (ttyout,565)
565	format (///,' type: ',/,
	' identification_confidence  material_ID_confidence   class_confidence', /,
        '     confidences are on a 0 to 9 scale,', /,
	'               0 = no confidence; information only',/,
	'               1 = position and shape match only',/,
	'               2 -9 = increasing diagnostic confidence',/,
	'               9 = highest confidence',//,
	'      class_confidence  is for the highest abundance class if a mixture',/)

	call crtin
	if (cmdverbose(-1) <= 2) write (ttyout,*) iopcon
	i = 1
	call wjfren (i,x,il)
	if (i >= 80 & il == 0) go to 564    # blank line, read again
	if (il==ihx || il==ihe)  {
		ic=il
		icrst = 1
		call rstart(icrst)
	...
	# 
	i = i -1
	if (iopcon(i:i+24) == 'identification_confidence') {
                           #            1111111111222222
		...
		if (i >= 80 & il == 0) {
			...
			call what (i)
			write (ttyout,*) ' no material ID confidence value (0-9) found'
			write (ttyout,*) ' STOP'
			stop
		}
		if ( x >= 0.0 && x <= 9.01 ) {
			idmatconfidence(1,imat) = x
			matidconfidence(imat)   = x / 9.0
			write (ttyout,*) ' material ID confidence=', matidconfidence(imat),'   imat=',imat
		} else {

			call what (i)
			write (ttyout,*) ' material ID confidence out of range.  should be >= 0, <= 9', x
			write (ttyout,*) ' '
			go to 564 
		}
		call wjfren (i,x,il)
		if (i >= 80 & il == 0) {
			...
			write (ttyout,*) ' no material class confidence value (0-9) found'
			write (ttyout,*) ' making class confidence = material ID confidence: ', idmatconfidence(1,imat)
			idmatconfidence(2,imat) =  idmatconfidence(1,imat)
			matidclassconfidence(imat) = idmatconfidence(2,imat) / 9.0
		}
		if ( x >= 0 && x <= 9 ) {
			idmatconfidence(2,imat) = x

			if (idmatconfidence(2,imat) < idmatconfidence(1,imat)) {

				write (ttyout,*) ' class confidence ', idmatconfidence(2,imat),
					' < material ID confidence: ', idmatconfidence(1,imat)
				write (ttyout,*) ' making class confidence = material ID confidence: '
			...
			call what (i)
			write (ttyout,*) ' class confidence out of range.  should be >= 0, <= 9', x
			write (ttyout,*) ' '
			go to 564 
		}
		...
		write (ttyout,*) ' NO identification_confidence keyword found'
		write (ttyout,*) ' found: ', iopcon(i:i+24)
		write (ttyout,*) ' STOP'
		stop
	}

### Now write history and info files

	write (lunhist,568) 

568	format('  history of material components and confidence to be added   ZZZZZ ')
...
3070	format (a,3x,a,'# Output base file name')
	...
	do ij = 1, maxmatcomps {
		...
		if (matcomponent(ij,imat)(1:30) != '                              ' ) {
                                                 #           111111111122222222223
                                                 #  123456789012345678901234567890

			write (lunhist,3072) mfile(imat)(1:43), imat, group(imat), 
					matcomponent(ij,imat)(1:30), matcompclass(ij,imat)(1:30),
					matcomponentabundance(ij,imat)
	...
	write (lunhist,3073) mfile(imat)(1:43), imat, group(imat),
			matidconfidence(imat), matidclassconfidence(imat)

3072	format (' material components: ',a43,' ',i4,' ',i3,' ',a30,' ',a30,' ', f7.4 )

3073	format (' material confidence: ',a43,' ',i4,' ',i3,' ', 'confidence=', f8.5,' class confidence=', f8.5)
...
463	format (i3, 3x, 'DN ',i6,' = ',f10.4, 9x, a,'# output bits, scale  ')
	...
	if ( group(imat) < 0 )  {   # case
	   ...
	   write (lundnscale, 289) imat, icase(imat), mfile(imat)(1:mfilelen), idn, x, enabledisablestr
289	   format('material= ',i5,'  case  ', i5, '  ', a, '  DN= ', i4, ' = ', f8.5, 4x, a)

	} else {  # group

	   write (lundnscale, 290) imat, group(imat), mfile(imat)(1:mfilelen), idn, x, enabledisablestr
290	   format('material= ',i5,'  group ', i5, '  ', a, '  DN= ', i4, ' = ', f8.5, 4x, a)
	}
